      $maxtime = 0
      $status = aws ssm get-parameter --region ${aws_region} --with-decryption --output text --name "/qumulo/${deployment_unique_name}/last-run-status" --query "Parameter.Value"
      ...
      Write-Host "*****CNQ Cluster Successfully Provisioned*****"